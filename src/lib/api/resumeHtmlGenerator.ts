...
interface ResumeData {
  ...
  certifications?: Certifications | CertificationItem[];
  firstName?: string;
  lastName?: string;
  email?: string;
  phoneNumber?: string;
 ...
 * @param {ResumeData | ResumeJsonWrapper} resumeData - The JSON resume data
 * @returns {string} - Complete HTML document as a string
 */
export function generateResumeHtml(resumeData: ResumeData | ResumeJsonWrapper): string {
  try {
    // Extract the jsonContent if it exists in the wrapper structure
    const data = 'jsonContent' in resumeData && resumeData.jsonContent 
      ? resumeData.jsonContent 
      : resumeData as ResumeData;
    
    // Format address from city, province, postalCode if available
    const formattedLocation = data.city || data.province || data.postalCode
      ? [data.city, data.province, data.postalCode].filter(Boolean).join(', ')
      : (data.contactInfo?.location || data.location || '');
      ...
      <div class="left-column">
        ...
        <div class="section">
          ...
          <div>
            ...
            ${renderContactItem('location', formattedLocation, 'M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z')}
 ...
 */
function renderSkills(skills: Skills | string[] | undefined): string {
  if (!skills) {
  ...
  if (Array.isArray(skills)) {
    skillsList = skills;
  } else if ('skillsList' in skills && Array.isArray(skills.skillsList)) {
 ...
 */
function renderEducation(education: Education | Education[] | undefined): string {
  if (!education) {
 ...
 */
function renderCertifications(certifications: Certifications | CertificationItem[] | undefined): string {
  if (!certifications) {
    return '';
  }
  ...
  } else if ('certificationsList' in certifications && Array.isArray(certifications.certificationsList)) {
    certList = certifications.certificationsList;
  }
  ...
  return `
    <div class="section">
      ...
      ${certList.map(cert => `
        <div class="cert-item">
          <h4>${cert.name || ''}</h4>
          <p>${cert.issuer || ''}${cert.dateReceived ? ` (${cert.dateReceived})` : cert.date ? ` (${cert.date})` : ''}</p>
        </div>
      `).join('')}
    </div>
  `;
 ...
 */
function renderProfessionalSummary(summary: ProfessionalSummary | undefined): string {
 ...
 */
function renderExperience(experience: Experience | ExperienceItem[] | undefined): string {
  if (!experience) {
  ...
  if (Array.isArray(experience)) {
    expList = experience;
  } else if ('experiences' in experience && Array.isArray(experience.experiences)) {
    expList = experience.experiences;
  }
 ...
 */
function renderProjects(projects: Projects | ProjectItem[] | undefined): string {
  if (!projects) {
    return '';
  }
  ...
  } else if ('projectsList' in projects && Array.isArray(projects.projectsList)) {
 ...
 */
function renderAchievements(achievements: string[] | string | undefined): string {
  if (!achievements) {
    return '';
  }
